...
enum CHNAGE_STATE {ADD, SUBTRACT}
var change_state = CHNAGE_STATE.ADD
...
onready var undoTimer: Timer = $Undo_Timer
...
func load_time():
	...
	if not openFile.file_exists("res://NDTimerFile.txt"): #"user://NDTimerFile.txt"
		self.use_default_time()
	var _Error_value : int = openFile.open("res://NDTimerFile.txt", File.READ)
...
func configure_time_formats():
	unit_time = ((second) + (minute * 60) + 
